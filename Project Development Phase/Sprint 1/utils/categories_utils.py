...
def add_category(name):
    db.execute("INSERT INTO categories (name) VALUES (:name)", {"name": name})
    db.commit()
    category_id = db.execute(
        "SELECT id from categories order by ROWID DESC limit 1"
    ).fetchone()[0]
    return category_id
...
def add_category_to_user(category_id, user_id):
    db.execute(
        "INSERT INTO user_categories (user_id, category_id) VALUES (:user_id, :category_id)",
